...
const AddIntern = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${backendUrl}/addintern`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      if (response.data === 'ok') {
        alert('Intern added successfully:', response.data);
        window.location.replace('/admin/InternManagement');
      }
    } catch (error) {
